      ...
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-wrap gap-8 p-8">
          {workflowData.nodes.map((node, index) => (
            <Card key={node.id} className="flex h-32 w-48 flex-col p-4 shadow-md">
        ...
        <Tabs defaultValue="settings">
          ...
          <TabsContent value="settings" className="space-y-4">
            <div className="space-y-2">
              ...
              <input type="text" className="w-full rounded-md border px-3 py-2" defaultValue="Sentiment Analysis" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Description</label>
              <Textarea className="min-h-[100px]" defaultValue="Analyzes the sentiment of the text" />
            </div>
            <div className="space-y-2">
...
function ChatPanel() {
  const [messages, setMessages] = useState([
    {
      role: "system",
      content: "I'm the Orchestrator Agent. How can I help you with your workflow?",
    },
  ])
  ...
  const handleSend = () => {
    ...
    setMessages([
      ...
      {
        role: "system",
        content: "I'll help you with that. What specific part of the workflow would you like to modify?",
      },
    ])
    setInput("")
  }
